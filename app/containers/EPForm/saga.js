...
function* searchProjectsSaga({ search }) {
  ...
  try {
    ...
    const projects = new List(
      res.data.map((projectData) =>
        Project.fromApiData(projectData, tasById, affiliateGroupsById)
      )
    );
...
export default function* epFormRootSaga() {
  yield takeLatest(
  ...
  yield takeLatest(searchProjectsActions.request.type, searchProjectsSaga);
