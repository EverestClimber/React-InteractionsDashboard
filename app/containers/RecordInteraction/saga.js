...
import { getHCPObjectives } from 'api/hcps';
...
function* fetchInteractionRecordingRequiredDataSaga() {
  ...
  try {
    const [resourcesResponse, projectsResponse] = yield [
      call(getResources),
      call(getProjects),
    ];
    ...
    const payload = {
      resources: new List(
        resourcesResponse.data.map((resourceData) =>
          Resource.fromApiData(resourceData)
        )
      ),
      projects: new List(
        projectsResponse.data.map((projectData) =>
          Project.fromApiData(projectData)
        )
      ),
    };
    ...
    yield put(
      fetchInteractionRecordingRequiredDataActions.error(error.message)
    );
    yield put(setLoading(false));
  }
}
...
function* fetchHCPObjectivesSaga({ hcpId }) {
  ...
  try {
    ...
    const hcpObjectives = new List(
      res.data.map((objectiveData) => HCPObjective.fromApiData(objectiveData))
    );
...
function* recordInteractionSaga({ interaction }) {
  try {
    ...
    yield put(setSuccessAction(true));
  } catch (error) {
...
export default function* recordInteractionRootSaga() {
  ...
  yield takeLatest(
    fetchHCPObjectivesActions.request.type,
    fetchHCPObjectivesSaga
  );
  yield takeLatest(
    recordInteractionActions.request.type,
    recordInteractionSaga
  );
  yield takeLatest(
